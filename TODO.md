...
- [ ] **(refactor)** Merge menu + Commander definitions into one sub-folder tree under `src/commands/`. Each area's parent lives at `src/commands/<area>.ts` and imports children from `src/commands/<area>/<child>.ts`. Menu wraps every Cmd in a `perm` check on dispatch, so the C1 admin-gate fix becomes a one-line `perm: isAdmin` per command.
- [ ] **C1 — Admin commands have no CLI-level admin gate** (`src/cli.ts:208-269`). After the restructure, set `perm: isAdmin` on every admin command and delete the duplicate direct-Commander definitions in `cli.ts`.

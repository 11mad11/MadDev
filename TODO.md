...
- [x] **H11 — Documentation refresh** (`CLAUDE.md`, `README.md`, `docs/install.md`, `docs/cli-reference.md`, `docs/client.md`, `systemd/sshd_config.snippet`, `docs/ca.md`, `docs/revocation.md`). Removed stale `ssh -w` / `otp` Linux user / daemon-side socat / `/etc/mad/groups` / "Networking" menu references. Added Windows port pointers, `mad doctor`, and the actual subcommand list reflecting `tap`/`tun`/`cert`. Updated wire-ops list to use `tap-allocate` / `ca-krl` / `refresh-cert` (no `consume-otp`).
- [x] **C3 — Rust FFI exports panic across C ABI (UB)** (`native/windows-tap/src/lib.rs:39-404`). Wrap every `#[no_mangle] extern "C"` body in `std::panic::catch_unwind(AssertUnwindSafe(|| { ... }))` and translate panics into `set_last_error` + the function's error sentinel. *(Added `ffi_guard("label", default, || { ... })` helper; every FFI export now runs inside it.)*
- [x] **C4 — `try_recv` doesn't reset `pending` on error → stuck pump** (`native/windows-tap/src/tap_win6.rs:92-130`). Clear `read.pending` and reset the event on every failure return. *(Both error paths after `WaitForSingleObject` and `GetOverlappedResult` now clear `read.pending = false` before returning Err, so the next `try_recv` re-issues a fresh ReadFile. Comment on `issue_read` corrected — the event is manual-reset; ReadFile resets it when it issues, not Wait.)*
